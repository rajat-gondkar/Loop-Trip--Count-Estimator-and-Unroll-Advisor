...
## Submission Usability Update

**Goal:** Make the project easier to run for evaluators from either the terminal or frontend.
...
- `static/app.js`
- `static/styles.css`
- Required C test inputs under `tests/c/`

**Commands run:**
- `LLVM_BIN=/opt/homebrew/opt/llvm@17/bin ./scripts/build.sh`
- `LLVM_BIN=/opt/homebrew/opt/llvm@17/bin ./scripts/run_tests.sh`
...
**Results observed:**
- Added `scripts/build.sh` and `scripts/run.sh` so setup can use one command for build and one command for the Flask frontend.
- Updated `scripts/build.sh` and `scripts/run.sh` to auto-detect common LLVM 17 locations, including Homebrew `llvm@17`, while still allowing `LLVM_BIN` overrides.
- Added missing required C test inputs for terminal evaluation.
- The frontend now includes a `Run 6 Tests` button that loads representative loop examples and runs analysis without requiring terminal commands.
- Terminal test result tables are written under `tests/results/`, with `tests/results/mixed.txt` as the most complete sample.
